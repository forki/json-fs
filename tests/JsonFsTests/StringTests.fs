...
[<Fact>]
let ``a string containing an invalid escape sequence will throw an exception``() =
    (fun() -> Json.parse "\"abc\d\"" |> ignore) |> should throw typeof<UnexpectedJsonException>

[<Fact>]
let ``a string containing an invalid unicode hex sequence will throw an exception``() =
    (fun() -> Json.parse "\"\ua00z\"" |> ignore) |> should throw typeof<UnexpectedJsonException>